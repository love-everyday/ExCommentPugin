...
class extDocumentCommand: NSObject, XCSourceEditorCommand {
    ...
    let fourSpaceStr    = "    "
    let threeCommentStr = "///"
    let startDocumentStr = "/**"
    let endDocumentStr = "*/"
    let descriptionStr  = "<#Description#>"
    let parameterStr    = " - Parameters "
    ...
    func insertFuncDoc(at index: Int, withLine line: String, inLines lines: NSMutableArray) {
        ...
        lines.insert(spaceStr + endDocumentStr, at: index)
        addLineCount += 1
        
        if line.funcStrHasReturnValue() {
            if hasDoc(at: index-1, withPrefix: spaceStr + returnsStr, inLines: lines) {
                return
            }
            
            let returnValueStr = spaceStr + returnsStr + "<#return value description#>"
        ...
        if paramNames.count > 0 {
            ...
            for paramName in paramNames.reversed() {
                ...
                let preParam = spaceStr + "- parameter " + paramName
                
                if hasDoc(at: index-1, withPrefix: preParam, inLines: lines) {
                    return
                }

                let paramDoc = preParam + ": " + "<#\(paramName) description#>"

                lines.insert(paramDoc, at: index)
            }

            addLineCount += paramNames.count
        }
        
        
        let preStart = spaceStr + startDocumentStr
        if hasDoc(at: index-1, withPrefix: preStart, inLines: lines) {
            return
        }
        
        let funcDoc = preStart + " " + descriptionStr
        if addLineCount <= 1 {
            let doc = funcDoc + " */"
            lines.replaceObject(at: index, with: doc)
            return
        }
        lines.insert(spaceStr, at: index)
        lines.insert(funcDoc, at: index)
